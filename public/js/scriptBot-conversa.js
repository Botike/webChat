...
// função para input vindo dos botões
const tdb_txt = (textDigitado) => {
...
// função para tratar links de botão
const tdb_link = (linkDigitado) => {
    pegaMensagemBPInsertaTemplate(linkDigitado);
    window.open(
        linkDigitado, '_blank'
      );
};

// função para tratar payload de botão
const tdb_payload = (payloadDigitado) => {
    pegaMensagemBPInsertaTemplate(payloadDigitado);
    console.log('Só testando: ', payloadDigitado)
};
...
const pegaMensagemBPInsertaTemplate = async(textEn_viado) => {
    ...
    fetch(uri, {
        ...
        // Aqui começa os tipos de features
        .then(function(retorno2) {
            ...
            for (var i = 0; i < lista.length; i++) {
                ...
                if (element.type == 'demorinha') {
                    ...
                    const template = templateDelay(demorinha, 'bp');
                    injetaTemplatenoChat(template);
                    ...
                    var carroCarBotoes = element.items.actions;
                    const template = templateCarousel({
                        carroCardTit, 
                        carroCardSubtit, 
                        carroCardImag, 
                        carroCarBotoes}, 'bp');
